...
class ProjectManager:
    ...
    def _extract_visualization_reference_rows(self, design_spec_path: Path) -> dict[str, str]:
        """Return page -> chart key rows declared in Design Spec §VII."""
        if not design_spec_path.exists():
            return {}

        rows: dict[str, str] = {}
        in_section = False
        for line in design_spec_path.read_text(encoding="utf-8", errors="replace").splitlines():
            if line.startswith("## "):
                in_section = line.startswith("## VII.")
                continue
            if not in_section:
                continue

            page_match = re.search(r"\b(P\d{2})\b", line)
            chart_match = re.search(r"templates/charts/([A-Za-z0-9_]+)\.svg", line)
            if page_match and chart_match:
                rows[page_match.group(1)] = chart_match.group(1)
        return rows

    def _extract_page_charts(self, spec_lock_path: Path) -> dict[str, str]:
        """Return page -> chart key rows declared in spec_lock.md ## page_charts."""
    ...
    def _validate_page_charts_handoff(self, project_path: Path) -> list[str]:
        ...
        actual = self._extract_page_charts(project_path / "spec_lock.md")
        errors: list[str] = []
        for page, chart_key in expected.items():
            actual_key = actual.get(page)
            if actual_key != chart_key:
    ...
    def import_sources(
        ...
        for item in source_items:
            ...
            if copy:
                ...
                effective_move = True
                print(
                    f"note: {source_path} is inside the viettel-ppt-master repo; moved "
