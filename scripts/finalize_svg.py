...
from apply_brand_chrome import process_project as process_brand_chrome
...
def finalize_project(
    ...
    # Optional deck-level cleanup/chrome runs before image embedding so any
    # injected logo is embedded by the normal image pipeline.
    if brand_chrome or strip_comments:
        if not quiet:
            label = brand_chrome or "none"
            safe_print(f"[brand] Applying brand chrome={label}, strip_comments={strip_comments}...")
        changed = process_brand_chrome(
            project_dir,
        ...
        if not quiet:
            safe_print(f"      {changed} SVG file(s) updated")
...
Examples:
  ...
  %(prog)s projects/my_project --brand-chrome viettel --strip-comments
...
Aliases (still accepted):
  crop-images, fix-aspect, embed-images  → all map to align-images
    ...
    parser.add_argument('--brand-chrome', choices=['viettel'], default=None,
                        help='Inject deck-level brand chrome into every SVG page')
    parser.add_argument('--strip-comments', action='store_true',
                        help='Remove XML comments from finalized SVG files')
