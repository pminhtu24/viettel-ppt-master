...
This directory contains user-facing scripts for conversion, project setup, SVG processing, native PPTX export, and web-image acquisition.
...
- `scripts/image_sources/`: web-image provider and download helpers used by `image_search.py`
- `scripts/template_import/`: internal PPTX reference-preparation helpers used by `pptx_template_import.py`
- `scripts/svg_finalize/`: internal helpers reused by the native PPTX converter
...
python3 scripts/svg_quality_checker.py <project_path>  # final scan
...
```bash
python3 scripts/image_search.py "modern workspace" --filename workspace.jpg -o projects/demo/images
...
- Prefer the unified entry points `project_manager.py`, `svg_to_pptx.py`, and `image_search.py`
- Export native PPTX directly from `svg_output/`
