...
- **Fonts**: under `viettel_default`, use exactly `"FS Magistral"`; do not append Arial/Calibri/Aptos or another fallback. Only `custom_override` stacks MUST end with a pre-installed family (Microsoft YaHei / SimSun / Arial / Times New Roman / Consolas …). `@font-face` is FORBIDDEN. Full rule: [`strategist.md §g`](strategist.md).
