...
|--------------|---------|-----------|
| Project creation + brand profile confirmed | **Strategist**: Eight Confirmations + Design Spec | Web Image Acquisition or Executor |
...
> **Execution discipline**: This is the last BLOCKING checkpoint in the pipeline. After confirmation, complete the Design Spec and proceed to any required web-image acquisition, SVG authoring, and post-processing without further pauses.
...
**Hard rule**: For `viettel_default`, the Viettel palette is truth. Keep Viettel red `#EE0033`, white/approved-gray surfaces, and dark-neutral text. Deep blue `#12436D` may appear only in chart, diagram/infographic, icon marks, and cataloged builtin backgrounds whose index item sets `deep_blue_background: true`; it remains forbidden for text, cards, rails, footers, dividers, ad-hoc backgrounds, and unregistered decoration. A requested color is interpreted as a compatible data/illustration accent when possible and does not unlock Viettel. Only an explicit `custom_override` may make user/template colors the primary brand palette.
...
> **Mandatory rules when choosing B**:
>
...
> **Viettel typography default (HARD rule for this skill).** Unless the user explicitly requests a non-Viettel brand override, report the Typography Plan in this form: "Locked font set: FS Magistral uses three static TTF faces; no synthetic font faces: Title / KPI / emphasis — FS Magistral Bold (`FS Magistral-Bold.ttf`); Body / caption / source — FS Magistral Book (`FS Magistral-Book.ttf`); Subtitle / secondary label — FS Magistral Medium (`FS Magistral-Medium.ttf`). SVG uses `700` / `400` or omitted / `500` only to select those faces. PPTX writes the exact Windows typeface and embeds every used face. Arial/Calibri substitution is forbidden." State this fixed rule for visibility but do NOT ask the user to choose or approve a typeface. Always name the physical face and file; never rename Book as Regular, propose alternative font combinations, or substitute generic PPT-safe stacks.
>
> **Viettel face selector lock (HARD rule).** SVG `700` selects FS Magistral Bold, `400` or omitted selects FS Magistral Book, and `500` selects FS Magistral Medium. Do not use `600`, `800`, or `900` in normal Viettel runs.

> The generic pairing guidance below applies only after an explicit non-Viettel override. In the default Viettel flow, hierarchy comes from the assigned static face, size, spacing, and color.
...
For **new Viettel decks**, use the role ranges and starting slots in [`viettel_default/design_spec.md §IV`](../templates/layouts/viettel_default/design_spec.md#iv-typography-system): **20px body for dense content / 24px for standard content**. Copy that Viettel ramp into the project's Design Spec and derive its `spec_lock.md` slots consistently. Keep page numbers unchanged and budget chart labels/footnotes separately. Existing project locks, mirror-template fidelity, and explicit user sizes take precedence; do not add another increase to an existing 22px or 24px body. Recalculate text-fit budgets at the chosen sizes without changing wrap/overflow rules.
...
| **C** | Web-sourced | Real-world reference imagery, editorial support, stock-style needs (no API key required for default providers) |
| **D** | Placeholders | Images to be added later |

AI image generation is not supported. If the user requests it, offer web sourcing, a user-provided file, or a placeholder.
...
| **Acquire Via** | `web` / `user` / `placeholder` — drives Step 5 dispatch |
...
| **Reference** | Free-form visual intent description; Image_Searcher converts web intents into provider queries |
...
🚧 **GATE — before writing §VIII Image Resource List**: when image approach is B/C/D (anything other than A "no images"), this is a three-layer hard requirement, not a suggestion:
...
- The source has major headings or numbered parts (`I`, `II`, `III`, `Part`,
  `Chapter`, `Section`, large H1/H2 clusters).
- The deck has 8+ planned slides and naturally splits into at least 2 distinct
  narrative blocks.
- The story changes mode, such as context -> current state, problem -> solution,
  analysis -> recommendation, solution -> roadmap, roadmap -> conclusion.
- A dense sequence of 3-5 content slides needs a clear transition before the
  next topic.

Divider rules:

- A divider must have an independent title and optionally one short subtitle;
  it is not a blank decorative slide.
- Do not create a divider before every small subsection; use one per major
  narrative block.
- For decks with 8-12 slides, target 1-3 dividers excluding cover/ending when
  the source structure supports them.
- For decks with 13+ slides, target 2-5 dividers when the source structure
  supports them.
...
Next step:
- Run Web Image Acquisition when the resource list contains `Acquire Via: web`
...
Next step:
- Run Web Image Acquisition when the resource list contains `Acquire Via: web`
- Then invoke Executor (adaptive Viettel composition for pages without layout inheritance)
```
