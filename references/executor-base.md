...
- **Controlled package ownership**: SVG generation runs in the main agent for main-agent packages, audited `parallel_runtime=main_agent_packages`, or page-level recovery after a failed spawn. When `generation_mode=chapter_parallel` selects `parallel_runtime=spawn_subagent`, sub-agent SVG authorship is allowed only for one assigned package per isolated sub-agent, with staging output, package report, and main-agent merge + validation. The main agent must not edit a running package or overwrite checker-clean pages preserved by partial merge.
- **Generation rhythm**: lock global design context first. Read `spec_lock.md ## generation` before SVG authoring. Default production mode is `generation_mode=serial`, `parallel_runtime=none`, `concurrency=1` for confirmed decks of 15 slides or fewer. `chapter_parallel` is used only when `spec_lock.md` says `mode: chapter_parallel` (confirmed page count is 16+ or explicit user parallel request), and concurrency resolves from the number of eligible sub-agent packages. Executor MUST run the parallel preflight gate before the first SVG only for `mode: chapter_parallel`. If `spawn_subagent` is unavailable or fails before package work starts, keep `generation_mode=chapter_parallel`, set `parallel_runtime=main_agent_packages`, and report the exact fallback reason before main-agent package execution. Ad hoc batches (e.g., 5 at a time) are still forbidden.
...
7. Run `python3 scripts/parallel_generation.py prepare-subagents <project_path>` before the first SVG. This creates run metadata, package prompts, staging dirs, and a `spawn_subagent` runbook.
8. Print a `Parallel Runtime Decision` checkpoint before SVG authoring. It must include generation mode, runtime (`none`, `spawn_subagent`, or `main_agent_packages`), run id, sub-agent package count, main-agent package count, and fallback reason.
9. If `spawn_subagent` is callable and the run manifest has `subagent_groups`, spawn only those groups by passing each package's `spawn_subagent_request.prompt` to `spawn_subagent(prompt=...)`. Main-agent packages remain in the main agent. Call one sub-agent per `subagent_groups` item; never give one sub-agent multiple package prompts. If the tool is unavailable, report `fallback_reason: spawn_subagent unavailable in active runtime` before `main_agent_packages` execution. A fallback claim is invalid unless the preflight gate was run and the runtime decision was printed.
10. Each sub-agent must read its generated package prompt, write SVGs only into `<project_path>/parallel_generation/runs/<run_id>/work/<group_id>/svg_output/`, run per-page checker on staged SVGs, and write `package_report.json`. It gets one consolidated repair pass per page; a no-op edit, repeated checker errors, or remaining errors after that pass must produce `status: "failed"` with `checker_errors` and end the package. Direct ad hoc prompts are a workflow violation; if `spawn_subagent_request.prompt`, `prompt_file`, `svg_output_dir`, or `package_report` is missing, stop and rerun `prepare-subagents`.
11. Call `spawn_subagent(prompt="<generated prompt>")` once per `subagent_groups` item. Multiple calls in the same turn are scheduled in parallel; there are no task IDs or polling. Use `--concurrency <n>` only when the user explicitly wants smaller batches.
12. The main agent must not author slides assigned to a running sub-agent; it may author standalone `main_agent_groups`.
13. After all spawn responses return, inspect every response and report. If every package passed, run strict merge. If any spawn aborted/failed or has a missing/invalid report, do not respawn; run `merge --allow-partial`.
14. Read `partial_merge_report.json`. The main agent creates only `missing_pages` and rewrites only `rejected_pages` in `svg_output/`; it must not modify `merged_pages`. `conflict_pages` block recovery unless intentional replacement uses `--replace-output`.
15. Run `python3 scripts/parallel_generation.py validate <project_path>` after all recovery pages pass their checker.
