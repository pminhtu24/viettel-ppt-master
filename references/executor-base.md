...
**Faithful-report closed-source lock**: when `content_mode.mode: faithful_report`, look up the current `P<NN>` under `page_sources` and read those exact ids and facts from `source_inventory.json`. Do not rely on an earlier summary or on memory. Copy facts directly—never add, calculate, or derive content. Preserve source order and all values/qualifiers. Every source-backed text leaf resolves to exactly one fact through inherited or local `data-source-ids="SRC01-B0001" data-fact-ids="SRC01-B0001-F01"`; one fact may span several text elements for line breaking. Chart labels, values, tables, and callouts follow the same rule. Source images use `data-content-kind="source_asset"` plus `data-source-ids`. Only literal Viettel branding may use `data-content-kind="brand_chrome"`; page-number-only text uses `data-content-kind="page_number"`. These attributes are internal QA metadata and must never be rendered as text. If a mapped block cannot fit, split the slide and update `design_spec.md` and `spec_lock.md` before continuing—never drop the block or shrink below the text-fit floor.
