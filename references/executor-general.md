...
Normal Viettel runs use the locked anchors from `spec_lock.md`; reflow or split crowded pages instead of shrinking them.

```
Title layer      → 38px   bold, theme color or white
Subtitle layer   → 32px   medium weight, secondary color
Body layer       → 26px   regular, dark gray
Annotation layer → 17px   regular, muted color, bottom-aligned
```
