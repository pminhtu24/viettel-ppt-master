...
| Area | Content | Font Size (Viettel default @ body=26px) | Style |
|------|---------|-----------|-------|
| Icon row | data-icon icon | 32x32 | Theme color |
| Metric name | "Monthly Active Users" | 17px (caption anchor) | Gray #64748B |
| Core number | "1.2M" | 39-52px (hero number band) | Bold, dark color |
| Trend annotation | "+12.3% vs last month" | 17px (annotation anchor) | Green=up / Red=down |

> Normal Viettel runs keep these anchors and split or reflow dense content. Ratio-based downsizing applies only to an explicit `custom_override` baseline.
