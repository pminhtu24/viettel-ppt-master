...
- If `mode: chapter_parallel` and OpenClaw exposes `sessions_spawn` / `sessions_yield`, spawn packages listed in `run_manifest.subagent_groups` as isolated sub-agents before SVG authoring for those packages. Use one spawn call per package; default concurrency equals the number of eligible sub-agent packages
