...
5. Sinh deck: sau khi bạn xác nhận, mặc định skill dùng `chapter_parallel` để chia deck theo chapter/package; nếu OpenClaw hỗ trợ `sessions_spawn`, các package độc lập có thể chạy bằng sub-agent song song, còn từng package vẫn sinh tuần tự để giữ continuity.
...
- Cách dùng hình ảnh.
- Chế độ sinh slide: mặc định `chapter_parallel`; có thể đổi về `serial` mode cũ nếu bạn yêu cầu rõ.
...
1. Tạo work packages bằng `python3 scripts/parallel_generation.py plan <project_path> --concurrency 2`.
2. Chuẩn bị prompt và staging bằng `python3 scripts/parallel_generation.py prepare-subagents <project_path> --concurrency 2`.
3. Nếu OpenClaw có `sessions_spawn` / `sessions_yield`, mỗi sub-agent nhận một chapter/package và ghi SVG vào staging riêng: `parallel_generation/runs/<run_id>/work/<group_id>/svg_output/`.
...
Nếu runtime không hỗ trợ sub-agent, skill phải in `Parallel Runtime Decision` kèm lý do fallback rồi mới được sinh serial. Dù chạy parallel hay serial, SVG vẫn được viết theo từng slide/package, không sinh bằng batch template/codegen hàng loạt.
